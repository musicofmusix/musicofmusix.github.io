...
date: 2026-03-02
abstract: >
 Digital Vinyl System (DVS) allows DJs to use a real turntable and vinyl record to manipulate digital audio for mixing and scratching. While typically implemented using a specialised record + hardware audio interface or an accelerometer, mouseDVS is a low-cost proof-of-concept that uses an ordinary optical mouse! Simply position the mouse on top of a good tracking surface (e.g., a felt slipmat), open the web app on a computer, and start playing. Made with HTML+Tailwind CSS (design), JS (logic).
